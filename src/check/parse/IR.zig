...
pub const NodeStore = struct {
    ...
    pub fn addBody(store: *NodeStore, body: Body) BodyIdx {
        ...
        if (body.whitespace) |ws| {
            store.extra_data.append(ws) catch exit_on_oom();
        }
    ...
    pub fn addHeader(store: *NodeStore, header: Header) HeaderIdx {
        ...
        switch (header) {
            .app => |app| {
                ...
                store.extra_data.append(app.platform) catch exit_on_oom();

                for (app.packages) |p| {
                    store.extra_data.append(p.id) catch exit_on_oom();
                }
                for (app.provides) |p| {
                    store.extra_data.append(p) catch exit_on_oom();
                }
            },
            else => {},
        }
        ...
        switch (statement) {
            ...
            .import => |i| {
                ...
                if (i.qualifier_tok) |tok| {
                    lhs.qualified = 1;
                    store.extra_data.append(tok) catch exit_on_oom();
                }
                if (i.alias_tok) |tok| {
                    lhs.aliased = 1;
                    store.extra_data.append(tok) catch exit_on_oom();
                ...
                for (i.exposes) |e| {
                    store.extra_data.append(@as(u32, @intCast(e))) catch exit_on_oom();
                }
            },
            .type_decl => |_| {},
            .type_anno => |_| {},
        ...
        switch (node.tag) {
            .app_header => {
                ...
                while (position < (rhs.num_packages + 1)) {
                    ...
                    store.scratch_tokens.append(data[position]) catch exit_on_oom();
                    position += 1;
                }
        ...
        switch (node.tag) {
            ...
            .import => {
                ...
                while (extra_data_pos < extra_data_end) {
                    store.scratch_tokens.append(store.extra_data.items[extra_data_pos]) catch exit_on_oom();
                }
