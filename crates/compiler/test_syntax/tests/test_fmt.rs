...
mod test_fmt {
    ...
    fn expr_formats_to(input: &str, expected: &str) {
        Input::Expr(input.trim()).check_invariants(check_formatting(expected.trim()), true, false)
    }

    fn expr_formats_same(input: &str) {
        Input::Expr(input.trim()).check_invariants(check_formatting(input.trim()), true, false)
    }
