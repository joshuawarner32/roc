...
fn parse_once_and_extract_error(text: &str) -> Option<String> {
    ...
    let res = std::panic::catch_unwind(|| parse_once(text));
...
fn parse_once(text: &str) -> Option<String> {
    ...
    // make sure we can get to the expect_atom state (throw it away)
    let _ = state_to_expect_atom(&state);
    ...
    if let Some(msg) = state.messages.first() {
        ...
        return Some(format!("{:?}: at {}", msg.kind, formatted_frames));
    }
...
fn candidate_minimizations(s: String) -> Box<dyn Iterator<Item = Update>> {
    ...
    let line_removals = (1..=line_count)
        .rev()
        .map(move |n| {
            let line_offsets_clone = line_offsets.clone();
    ...
    let line_selects = (1..line_count - 1)
        .map(move |n| {
            assert!(n > 0);
